...
const Engine = (() => {
  ...
  function simDay(day, p, equityRef){
    ...
    const sig = strat==='meanrev' ? bandSigma(bars, vwap) : null;
    const tstr = (strat==='meanrev' && p.trendFilter) ? trendStrength(bars, vwap) : null;
    const held=new Array(n).fill(0);

    // precompute the trend stance if needed
    const tdes = strat==='trend' ? trendDesired(bars, vwap, p.entryMode, p.confirmBars) : null;
    ...
    for(let i=0;i<n;i++){
      ...
      if(pos!==0 && (p.stopMult>0 || p.tpMult>0 || strat==='meanrev')){
        const dref = dist[i]>0?dist[i]:1e-9;
        // stop distance & target distance in PRICE units
        const stopPx = p.stopMult>0 ? p.stopMult*dref : Infinity;
        if(strat==='meanrev'){
          // TP = return to VWAP; SL = stretched stopMult past entry (away from VWAP)
          ...
          if(hitTP){ tradeTo(0, vwap[i], i); held[i]=0; continue; }
      ...
      // ---- entry / reversal logic per strategy ----
      if(strat==='meanrev'){
        if(pos===0){
          // trend filter: skip entries on strongly-trending sessions
          const blocked = p.trendFilter && tstr && tstr[i] > p.trendMax;
          if(!blocked){
            const dev = bars.c[i]-vwap[i];           // +ve = above VWAP
            let long=false, short=false;
            if(p.bandMode==='sigma'){
              const s = sig[i]>0 ? sig[i] : 1e-9;
              if(dev >=  p.sigmaMult*s) short=true;   // far above -> fade short
              if(dev <= -p.sigmaMult*s) long=true;    // far below -> fade long
            } else { // 'avgdist'
              const dref = dist[i]>0?dist[i]:1e-9;
        ...
        // (exits handled by TP/SL above)
      } else { // trend
        const want=tdes[i-1];              // next-open execution
        if(want!==pos){
          // if we have SL/TP active and just got stopped, tdes still says 'want';
          // allow re-entry only when flat or on a genuine opposite signal
          tradeTo(want, bars.c[i], i);
        }
      }
      held[i]=pos;
    }
